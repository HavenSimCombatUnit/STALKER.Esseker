...
_VD_markerTC = createMarker ["TraderCamp", _VD_TC_Spawn];
_VD_markerTC setMarkerShape "ICON";
_VD_markerTC setMarkerType "hd_dot";
_VD_markerTC setMarkerText "Trader Camp";
hint "Hello Survivors, we have set up our Tradingpost at this location!";


//1 FIREPLACE, TENTS, CAMONETS SPAWNER
_fireplace = "Campfire_burning_F" createVehicle _VD_TC_Spawn;
...
_VD_TC_Spawn = [getPosATL _player, VD_SpawnMinDist, VD_SpawnMaxDist, 0, 0, 0.1, 0,VD_Land_Blacklist_Area] call BIS_fnc_findSafePos;

_isFlatEmpty = !(_VD_TC_Spawn isFlatEmpty  [30, -1, 0.1, 30, -1, false] isEqualTo []);

_nearcamps = _VD_TC_Spawn nearObjects ["Campfire_burning_F", VD_TC_DistanceCheck];
_cntfp = count _nearcamps;


if (_isFlatEmpty) then {

  if (_cntfp == 0) then {call _VD_traderspawner}

  else {call _VD_TC_Check};
