...
// **************************************************************************
// Execute command chosen by player from Soolie's menu
// **************************************************************************
...
switch (_command) do
{
    ...
    case "getin": 
    {
        ...
        //hint "getin cmd";
        if (isnull (attachedTo _dog) or (_dog getVariable "vIsSwimming") )then  // if dog not already in (i.e., attached) to a vehicle.
        //if (isnull (attachedTo _dog)  ) then  // if dog not already in (i.e., attached) to a vehicle.
        {
            //hint "JBOY_dogExecutionAction getin command issued";
           _obj = objNull;
           ...
           if (isnull _obj) then  // if handler not in a vehicle, and not pointing exactly at a vehicle, then use nearest vehicle
           {
                _nearVehicles = [];
                _nearVehicles = nearestObjects [_handler, ["Ship","Air","LandVehicle"], 20];
                if (count _nearVehicles > 0) then
                {
                    _obj = _nearVehicles select 0;
                };
           };
           ...
           {
                 //if ((_handler distance _obj) < 31 or surfaceIsWater getpos _dog) then 
        ...
        if !(isnull (attachedTo _dog)) then  // Must be in/attached to vehicle
        {
            _dog setVariable ["vVehicle", objNull, true];
             if !(attachedTo (_dog) == objNull) then 
             {
               ...
               _dummy= [_dog, .3, "whimper1",""] execVM "JBOY_Dog\delaySay.sqf";
             };
        };
    };
    case "track": 
    ...
    case "speak": 
    {
       ...
       if (  ((getObjectTextures _dog select 0) find "dogBulletHole2")>=0 or ((getObjectTextures _dog select 0) find "dogBloodNeck2Hole")>=0) then
        {
            _dummy= [_dog, 2, selectRandom [ "boomerYelp", "boomerYelp2"], ""] execVM "JBOY_Dog\delaySay.sqf";
        } else {
        ...
        {
            params["_dog"];
            JBOY_sitrep_dog = _dog;
            _id = addMissionEventHandler ["Draw3D", {
                //drawIcon3D ["\A3\ui_f\data\map\markers\military\triangle_ca.paa",
                drawIcon3D ["iconAnimal",
                    //[0.000,0.322,1.000,0.5], //Color Blue
                    [0.73,0.24,0.11,1], // Color Red
                    [getPos JBOY_sitrep_dog select 0, getPos JBOY_sitrep_dog select 1, 1.25],
                    1, 1,
                    360,
                    "",
                    0,
