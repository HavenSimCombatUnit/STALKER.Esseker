...
// When dog makes Contact, attack command will target detected enemy (when no other obvious enemy pointed at by handler)
if (isNull _returnTarget) then
{
    if (_dog getVariable "vCommand" == "contact") then
    {
        _returnTarget = _dog getVariable "vTarget";
    };
};
if (isNull _returnTarget) then
{
