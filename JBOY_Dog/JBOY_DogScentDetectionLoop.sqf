...
//diag_log [vehicle _dog, vehicle _dog ==_dog, surfaceIsWater (getpos _dog)];
    ...
    {
        _scentMarkers = nearestObjects [_dog, ["Sign_Arrow_Large_Cyan_F"], _dog getVariable "vScentTrailDetectDistance"] ;
        ...
        // trail needs at least 2 markers...this sometimes happens when attacking prey, previous markers deleted, and only one new marker created for prey survivor
            _preyTrackingObj = (_scentMarkers select 0) getvariable ["prey",objNull];
            if (_preyTrackingObj == objNull) then
            {
                deleteVehicle (_scentMarkers select 0);
            } else
            {
                _trail = _preyTrackingObj getVariable ["trail",[]];
                _trailIsActive = _preyTrackingObj getVariable "trailIsActive";
               //if ((count _trail) > 1 and _trailIsActive) then             
                if ((count _trail) > 1 ) then             
                {
                    if (_trailIsActive) then
                    {
...
// *********************************************************************************************
// if intercept trail midway, delete all scentmarkers older than current marker just found.
...
{  
    ...
    */
    ...
    // tag the prey being chased with dog chasing it
    _preyTrackingObj setVariable ["trackedByDog",_dog,true];
