...
if !(_handler == objnull) then
{
	...
	_dog setVariable ["vSide", side _handler, true]; 
} else
{
	_dog setVariable ["vSide", sideEnemy, true]; // Pack dogs don't have handlers, and side set to sideEnemy so they attack all humans
};
...
{
    _dog setVariable ["vHandler", objNull, true];  
...
//_dogSide = [_dog] call JBOY_Dog_getDogSide; // THIS IS FUCKING UP DOG PACK CREATION FOR SOME REASON.
