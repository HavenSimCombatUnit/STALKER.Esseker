...
if (!isNull _handler) then
{
	...
	_handlerGroupUnits = units group _handler;
    // **************************************************************************
	...
	if (player == _handler) then
	{
		_handlerIsPlayer = true;
	};
}; 
            ...
            } ELSE 
            {
                if ( isNull(_dog getVariable "vPack") ) then
                {
                    ...
                    } else
                    {
						...
						{
							sleep (3 + random 4);
							_handler setBehaviour "SAFE";
							_handler moveTo getpos _handler;  // put ai back in unitReady = true and he will hopefully resume waypoints
							_dog setVariable ["vCommand", 'heel', true];  // put dog back into heel mode if handler is ai and all enemies dead
							_dmy = [_dog] spawn {params["_dog"];sleep 60; _dog setvariable ["vHits",0,true]; _dog setObjectTextureGlobal [0,_dog getVariable "vOriginalTexture"];}; // heal the dog after awhile because too much trouble to script ai healing his dog
						};
                    };
                } else
                {
   ...
   if ((_dog getVariable "vCommand") == "detain" and (alive _handler )) then
   {
    ...
    if ((!alive _handler ) and !(isnull _handler)) then
    {
        ...
        if !(isnull _handler) then
        {
            ...
            {
				//systemChat format ["handler dead, assign new one   _handlerIsPlayer=%1, _handler=%2", _handlerIsPlayer, _handler];
				if (!_handlerIsPlayer) then
				{
					{if (alive _x) then {_handler = _x};} foreach _handlerGroupUnits;
					//systemChat format ["new _handler=%1",  _handler];
					sleep 5;
					_dog setVariable ["vHandler",_handler,true];
					//_dmy = [_dog, _handler, 2.5] execVM "JBOY_Dog\JBOY_dogCommand.sqf"; 
					sleep 1;
					if (alive (_dog getVariable "vTarget")) then 
					{
						_dog setVariable ["vCommand", 'attack', true];
					} else{
