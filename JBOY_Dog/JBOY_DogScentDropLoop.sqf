...
params ["_prey","_trackingInterval","_markerDistance"];  // _prey is a canoper object
//diag_log ["in JBOY_DogScentDropLoop, _this=",_this];
// *************************************************************************************************
...
//hintc format ["%1",_droppedObjTypes];
/*
...
if (!(typeOf _droppedObj in ["Land_Can_Dented_F","Land_WaterPurificationTablets_F","Land_Matches_F","Land_Bandage_F","BloodSplatter_01_Small_New_F","MedicalGarbage_01_Injector_F","Land_PowderedMilk_F"])) 
then {_droppedObj setVectorUp [0,.9,0];};  // dropped objects shouldn't stand upright
...
//systemchat format ["000 quarry=%1, vehicle _quarry=%2, _lastscentpos=%3",_quarry,vehicle _quarry,_lastScentPos];
    ...
    // ****************************************************************
        ...
        // ****************************************************************
        if ((_quarry isKindOf "Man" and _durationAtPos > 10) or (random 100) > 90 ) then
        {
...
//systemChat format ["dropped %1",(_scentMarker getVariable "droppedObject")];
                ...
                //_droppedObj setCenterOfMass [[0,.2,0],0];
