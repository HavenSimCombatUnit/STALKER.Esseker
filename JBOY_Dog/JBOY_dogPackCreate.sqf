...
// Set Pack properties
// **************************************************************************************
//diag_log ["_pack=",_leader, _pack, typeOf _pack];
...
while {_cnt <= _dogCount} do
{
	...
	_dog setVariable ["vSide", sideEnemy, true]; // pack dogs attack every one
    nul = [_dog, objNull,0] execVM "JBOY_Dog\JBOY_dogCommand.sqf";
	_members = [];
//diag_log ["before",_dog, _pack, _members];
    _members = _pack getVariable "vPackMembers";
//diag_log ["after", _dog, _pack, _members];
    ...
    _n=[_dog] spawn {sleep 2; params["_dog"]; _dog setVariable ["vCommand", "followLeader", true]}; // Start pack member following pack leader
};
// **************************************************************************************
