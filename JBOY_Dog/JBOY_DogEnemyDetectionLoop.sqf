    ...
    
    {
        ...
        if (count _enemies > 0) then
        {
            ...
            if (_target != _prevTarget or (_targetPos distance _target) > 10) then
            {
                ...
                sleep .5;
				// If its an AI handler then change his behaviour to COMBAT and have him order dog to attack.
				if (_handler != player) then
				{
					sleep 10;
					_handler setUnitPOS "MIDDLE";
					{_x setBehaviour "COMBAT";} foreach units group _handler;
					_handler doWatch getPos _target;
					sleep (3 + random(4));
					_dog setVariable ["vTarget", _target, true];      // give dog a target ai object
					_dog setVariable ["vCommand", 'attack', true];     // command dog to attack
					_handler setUnitPOS "AUTO";
					sleep 1;
					//_handler setSpeedMode "FULL";
					_handler moveTo getpos _target;
				};
