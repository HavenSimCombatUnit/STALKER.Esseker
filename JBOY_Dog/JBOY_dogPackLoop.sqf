...
while {{alive _x} count _packMembers > 0 or alive (_pack getVariable "vPackLeader")} do
{
    ...
    //{    
        if ((_pack getVariable "vMovementType") == "roam") then
        {
            ...
            _nearTargets = [];
            _nearTargets = [getpos (_pack getVariable "vPackLeader"), ["CAManBase","Hen_random_F","Cock_random_F","Cock_white_F"], (_pack getVariable "vAttackPreyRadius")] call JBOY_getNearTargets;
        ...
        // movement type is "attack"
        {
            ...
            waituntil {sleep .5; ({alive _x} count _packMembers == 0 )|| ({alive _x} count _nearTargets) == 0 ||(_pack getVariable "vMovementType") == "roam"  }; 
